...
void CLanguageManager::LoadXML (const std::string& pathFile)
{
	...
	LOGGER->AddNewLog(ELOG_LEVEL::ELL_INFORMATION, "LanguageManager::LoadXML-> Parseando fichero de lenguaje: %s", pathFile.c_str());
    ...
    for (int i = 0; i < count; ++i)
    {
			...
			LOGGER->AddNewLog(ELOG_LEVEL::ELL_INFORMATION, "LanguageManager::LoadXML-> Añadido literal(%s,%s,[%f,%f,%f,%f],%s)",
	...
	if (m_Languages.find(id_language) != m_Languages.end())
	{
		...
		LOGGER->AddNewLog(ELOG_LEVEL::ELL_WARNING, "LanguageManager::LoadXML-> EYa se ha registrado un language con identificador %s", id_language.c_str());
	}
	else
	{
...
void CLanguageManager::SetXmlFile (const std::string& pathFile)
{
	...
	if (!exist)
	{
		...
		LOGGER->AddNewLog(ELOG_LEVEL::ELL_INFORMATION, "LanguageManager::SetXmlFile-> Se ha añadido el xml: %s", pathFile.c_str());
	}
...
void CLanguageManager::SetCurrentLanguage (const std::string& id)
{
	...
	if (it != m_Languages.end())
	{
		...
		LOGGER->AddNewLog(ELOG_LEVEL::ELL_INFORMATION, "LanguageManager::SetCurrentLanguage-> Se ha seteado como current el language %s", id.c_str());
	}
	else
	{
		LOGGER->AddNewLog(ELOG_LEVEL::ELL_WARNING, "LanguageManager::SetCurrentLanguage-> El language %s no esta registrado", id.c_str());
	}
}
