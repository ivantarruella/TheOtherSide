...
void CPhysicActor::SetDynamicActorState(bool disabled)
{
	if (!m_pPhXActor || !m_pPhXActor->userData)
		return;

	if (m_pPhXActor->getNbShapes() > 1)
    {
        for (size_t i = 0; i < m_pPhXActor->getNbShapes(); i++)
        {                   
            m_pPhXActor->getShapes()[i]->setFlag(NX_SF_DISABLE_COLLISION, disabled);
        }               
    }
    else
    {
    ...
    for (size_t i = 0; i < m_pPhXActor->getNbShapes(); i++)
    {
        m_pPhXActor->getShapes()[i]->setFlag(NX_SF_DISABLE_SCENE_QUERIES, disabled);
    }

    if (disabled)
    {
        m_pPhXActor->raiseBodyFlag(NX_BF_KINEMATIC);
        m_pPhXActor->putToSleep();
    }
    else
    {
        m_pPhXActor->clearBodyFlag(NX_BF_KINEMATIC);
...
bool CPhysicActor::isSleeping()
{
	return m_pPhXActor->isSleeping();
}
