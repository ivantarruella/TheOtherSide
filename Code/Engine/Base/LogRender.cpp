...
, m_fTotalAnimation(1.f)
, m_Quad2dColor(0.f,0.f,0.2f,0.7f)
, m_fAlpha(0.7f)
{
}
...
void CLogRender::SetVisible (bool flag)
{
	...
	if( !m_bAnimation)
	{
		if (m_bIsVisible == false && flag == true)
		{
			...
			m_LerpAnimator1D.SetValues(0.f,(float)m_uLinesPerPageAnimation, 0.05f,FUNC_CONSTANT);
			m_iDirecion = 1;
		...
		else if (m_bIsVisible == true && flag == false)
		{
			...
			m_bAnimation = true;
			m_iDirecion = -1;
			m_LerpAnimator1D.SetValues((float)m_uLinesPerPageAnimation,0.f, 0.1f,FUNC_CONSTANT);
		}
	}
...
void CLogRender::Render (CRenderManager* renderManager, CFontManager* fm, CColor color)
{
	if( m_bIsVisible )
	{
		...
		for (; it != itEnd; ++it)
		{
			...
			if (w > win_w)
			{
				...
				uint32 max_size = win_w -0 - m_WindowsPos.x - INC_X_TO_MESSAGE; //un margen de 0 por la izquierda
	...
	else
	{
		...
		action2Input->GetActionInfo(DOACTION_LOGGER, info, shortInfo);
