...
void CCheckButton::Render (uint32 num, CRenderManager *renderManager, CFontManager* fm)
{
	if( CGuiElement::m_bIsVisible)
	{
		...
		Vect2i l_OffsetV(0, (CGuiElement::m_uHeight/4 * num));
		if( CGuiElement::m_bIsActive )
		{
			if( m_eState == CBS_ON )
			{
				if (m_pOnTexture)
				{
					renderManager->DrawTexturedQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_pOnTexture);
				}
				else
				{
					renderManager->DrawQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_OnColor);
				}
			}
			else
			{
				if (m_pOnTexture)
				{
					renderManager->DrawTexturedQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_pOffTexture);
				}
				else
				{
					renderManager->DrawQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_OffColor);
				}
			}
		}
		else
		{
			...
			{
				renderManager->DrawTexturedQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_pDeactivatedTexture);
			}
			else
			{
				renderManager->DrawQuad2D(CGuiElement::m_Position+l_OffsetV,CGuiElement::m_uWidth,CGuiElement::m_uHeight, UPPER_LEFT, m_DeactivatedColor);
			}
		}
