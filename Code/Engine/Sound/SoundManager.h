...
public:
    ...
    //---------ACTION FUNCTIONS-------------------------------
    bool    PlayAction2D    (const std::string& action, float volume = 1.0f);
...
private:
	...
	bool m_bSoundON;
	bool m_bPause;
	//Acciones asociadas a ficheros de sonido
	std::map<tAction, tIdBuffer>	m_Buffers;
	//Ficheros de sonido: si dos acciones diferentes tienen el mismo sonido, 
	//no debemos cargar el sonido dos veces
	std::map<tSoundFile,tIdBuffer>	m_LoadedFiles;
