...
float g_MonochromeColorGrading = 0.1;
float g_BrownSepiaColorGrading = 0.2;
float g_GreenSepiaColorGrading = 0.2;
float g_BlueSepiaColorGrading = 0.1;
float g_ContrastColorGrading = 0.8;
...
float4 ColorGrading(float4 _Color)
{
	...
	float4 combine = (brownsepia *g_BrownSepiaColorGrading ) + (greensepia*g_GreenSepiaColorGrading )+ (bluesepia * g_BlueSepiaColorGrading )+ (monochrome4	* g_MonochromeColorGrading)+(g_ColorColorGrading * result2);
...
{
	float4 l_Color= tex2D(gDiffuseSampler, UV);
